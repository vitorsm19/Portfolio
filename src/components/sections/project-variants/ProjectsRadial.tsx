      ...
      <div className="max-w-6xl mx-auto px-6 lg:px-16 mt-12 pb-20 lg:pb-32">
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          {projects.map((p, i) => (
            ...
            >
              <Image src={p.logo} alt="" width={24} height={24} className={`w-5 h-5 object-contain transition-all duration-500 ${active === i ? '' : 'grayscale opacity-50'}`} />
          ...
          >
            <div className="flex flex-col lg:flex-row">
              ...
              <div className="lg:w-2/5 p-8 lg:p-10 flex flex-col justify-center relative">
                <span className="font-mono text-[11px] text-text-muted/20 tracking-[0.3em] mb-2">{pad(active + 1)}</span>

                <Image src={project.logo} alt="" width={80} height={80} className="w-16 h-16 object-contain mb-4" />

                <h3 className="text-3xl lg:text-4xl font-bold text-text-heading leading-tight">{project.title}</h3>
                <p className="mt-4 text-text-secondary/70 text-base leading-relaxed font-body">{project.description}</p>
                ...
                <a href={project.url} target="_blank" rel="noopener noreferrer" className="mt-8 inline-flex items-center gap-2 font-mono text-sm tracking-wider px-6 py-2.5 rounded-full bg-accent text-white hover:brightness-110 transition-all cursor-pointer self-start">
