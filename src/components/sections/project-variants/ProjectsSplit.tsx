            ...
            >
              ...
              <div className="lg:w-1/2 w-full rounded-xl overflow-hidden border border-white/[0.04] group-hover:border-accent/15 transition-colors duration-500">
                <div className="relative aspect-video">
                  ...
                  <span className="absolute top-4 right-4 text-white/20 text-lg group-hover:text-accent group-hover:translate-x-1 group-hover:-translate-y-1 transition-all duration-500">
                    &#8599;
                  </span>
                </div>
              </div>
              ...
              <div className={`lg:w-1/2 ${isEven ? '' : 'lg:text-right'}`}>
                <span className="font-mono text-[11px] text-text-muted/20 tracking-[0.3em] pe-2">
                  {pad(i + 1)}
                </span>
                ...
                <p className="mt-3 text-text-secondary/70 text-sm lg:text-base leading-relaxed font-body">
                  {project.description}
                </p>
                <div className={`flex flex-wrap gap-1.5 mt-4 ${isEven ? '' : 'lg:justify-end'}`}>
                  {project.tech.map((t) => (
                    <span
                      key={t}
                      className="font-mono text-[10px] tracking-wider uppercase px-2.5 py-1 rounded-full border border-white/[0.06] text-text-muted/40"
                    >
                      {t}
                    </span>
                  ))}
